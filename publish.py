...
def main():
    ...
    site = {
        ...
        "states": (jread(os.path.join(HERE, "state_votes.json"), {}) or {}).get("states", {}),
        "votes": (jread(os.path.join(HERE, "state_votes.json"), {}) or {}).get("votes", []),
        "states": {k: v for k, v in (cfg.get("states") or {}).items() if isinstance(v, dict) and v.get("v")},
